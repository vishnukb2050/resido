                            ...
                            <View style={[styles.section, { marginTop: 32 }]}>
                                ...
                                {sharedItems.map((item) => (
                                    <TouchableOpacity 
                                        ...
                                        onPress={() => {
                                            if (item.folder) {
                                                router.push({ pathname: '/folder-view', params: { id: item.folder.id, name: item.folder.name, isShared: 'true' } });
                                            } else if (item.file) {
                                                // Handle single file view if needed
                                            }
                                        }}
                                    >
                                        <View style={[styles.folderIconBox, { backgroundColor: '#94a3b8' }]}>
                                            <MaterialCommunityIcons name={item.folder ? "folder-account" : "file-outline"} size={24} color="#fff" />
                                        </View>
                                        <View style={styles.folderInfo}>
