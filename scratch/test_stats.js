...
async function test() {
    ...
    try {
        ...
        const securityStaff = await prisma.member.count({ where: { role: 'SECURITY_STAFF' } });
        const cleaningStaff = await prisma.member.count({ where: { role: 'CLEANING_STAFF' } });
        const adminStaff = await prisma.member.count({ where: { role: 'ADMIN_STAFF' } });
        const maintenanceStaff = await prisma.member.count({
            where: { role: { in: ['MAINTENANCE_STAFF', 'STAFF'] } }
        });
        ...
        const gatepassesApproved = await prisma.visitor.count({ where: { status: 'APPROVED' } });
        console.log("gatepassesCreated:", gatepassesCreated, "gatepassesApproved:", gatepassesApproved);

        console.log("All queries executed successfully!");
    } catch (err) {
        console.error("Error executing queries:", err.stack || err);
    } finally {
